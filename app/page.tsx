...
export default function Home() {
  ...
  const renderList = () => (
    <div className="flex h-full flex-col relative bg-background-dark">
      {/* Filter Pills */}
      ...
      <div className="no-scrollbar flex-1 overflow-y-auto px-4 pt-4 pb-24">
        <div className="flex flex-col gap-3">
          {displayedMigraines.map((migraine) => {
            ...
            // Highlight selected item on desktop/edit view
            const isSelected = editingId === migraine._id;

            return (
              <div
                key={migraine._id}
                onClick={() => startEdit(migraine)}
                className={`group relative flex cursor-pointer items-center justify-between rounded-xl border bg-surface-dark p-4 shadow-lg shadow-black/20 transition-all active:scale-[0.98] hover:border-primary/50 ${
                  isSelected 
                    ? "border-primary shadow-purple-900/20 ring-1 ring-primary/20" 
                    : "border-gray-800"
                } ${
      ...
      {/* Bottom Nav (Floating Action Button) */}
      <div
  ...
  const renderForm = () => (
    <div className="flex h-full flex-col overflow-hidden bg-background-dark">
      {/* Header */}
      <header className="flex-shrink-0 flex items-center justify-between px-6 py-6 border-b border-gray-800/50">
        <button
          onClick={() => {
            resetForm();
            setView("list");
  ...
  return (
    <div className="flex h-dvh w-full overflow-hidden bg-background-dark font-display text-text-dark">
      {/* Left Panel - List */}
      <div 
        className={`flex-col border-r border-gray-800 bg-background-dark w-full md:w-[400px] md:flex
      ...
      {/* Right Panel - Form */}
      <div 
        className={`flex-col flex-1 bg-background-dark relative w-full md:flex
        ${view !== 'list' ? 'flex' : 'hidden'}`}
      >
        {view === 'list' ? (
           <div className="hidden md:flex h-full flex-col items-center justify-center text-gray-500 gap-4">
             <div className="w-20 h-20 rounded-full bg-surface-dark border border-gray-800 flex items-center justify-center">
